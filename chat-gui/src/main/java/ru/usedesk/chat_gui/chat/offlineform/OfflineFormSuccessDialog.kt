...
internal class OfflineFormSuccessDialog private constructor(
    ...
    init {
        inflateItem(
            layoutInflater,
            container,
            chatR.layout.usedesk_dialog_offline_form_success,
            dialogStyle,
            ::Binding
        ).apply {
    ...
    companion object {
        fun newInstance(container: View) = OfflineFormSuccessDialog(
            ...
            UsedeskResourceManager.getResourceId(chatR.style.Usedesk_Chat_Offline_Form_Success_Dialog)
        )
    }

    class Binding(rootView: View, defaultStyleId: Int) : UsedeskBinding(rootView, defaultStyleId) {
        val tvClose: TextView = rootView.findViewById(chatR.id.tv_close)
    }
}